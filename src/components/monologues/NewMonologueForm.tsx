  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200/60 p-5 flex gap-3 items-end">
      <div className="flex-1">
        <label className="block text-xs font-medium text-gray-500 mb-1.5">{t("monologue.title.label")}</label>
        <input
          ...
          placeholder={t("monologue.title.placeholder")}
          value={title}
          onChange={(e) => setTitle(e.target.value)}
        />
      </div>
      <Button onClick={create} disabled={!title.trim()}>
        <Plus size={16} /> {t("monologue.create")}
      </Button>
    </div>
  );
}
